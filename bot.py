...
VC_IDS = [1353875050809524267,21409170559337762980,1353875404217253909,1353882705246556220]
...
logger = logging.getLogger("spiciest-sfw-compress")
_token_split_re = re.compile(r"[^a-z0-9]+")
_seed_gif_tags = ["waifu","neko","kawaii","cute","smile","blush","hug","kiss","cuddle","cosplay","maid","bikini","swimsuit","school_uniform","idol","thighs","stockings","panties_peek","underboob_hint","fanservice","flirty","teasing","dance","cheerleader","sundress","soft","petite","playful","romantic","beach"]
...
EXCLUDE_TAGS = ["loli","shota","child","minor","underage","young","schoolgirl","age_gap","futa","futanari","shemale","dickgirl","femboy","trap","gay","yaoi","male","man","boy"]
...
def add_tag_to_gif_tags(tag: str, GIF_TAGS, data_save):
    ...
    logger.debug(f"learned tag: {t}"); return True
...
if not GIF_TAGS: GIF_TAGS=["waifu"]
def save_data():
    try:
...
PROVIDER_TERMS = {
    "waifu_pics":["waifu","neko","cute","smile","bikini","swimsuit","maid","cosplay"],
    "waifu_im":["waifu","maid","cute","cosplay","bikini","thighs","hug","kiss"],
    "waifu_it":["waifu","cute","cosplay"],
    ...
    "danbooru_safe":["smile","blush","cute","cosplay","bikini","swimsuit"]
}
...
async def fetch_from_waifu_pics(session,positive):
    try:
        ...
        async with session.get(url,timeout=REQUEST_TIMEOUT) as resp:
            ...
            if not gif_url: return None,None,None,None
            if filename_has_block_keyword(gif_url): return None,None,None,None
            if contains_illegal_indicators(json.dumps(payload)+" "+(category or "")): return None,None,None,None
            extract_and_add_tags_from_meta(json.dumps(payload),GIF_TAGS,data)
            return gif_url,f"waifu_pics_{category}",payload,None
    except Exception as e:
...
async def fetch_from_waifu_im(session,positive):
    try:
        ...
        async with session.get(base,params=params,headers=headers or None,timeout=REQUEST_TIMEOUT) as resp:
            ...
            if not images: return None,None,None,None
            img=random.choice(images)
            gif_url=img.get("url") or img.get("image") or img.get("src")
            if not gif_url: return None,None,None,None
            if filename_has_block_keyword(gif_url): return None,None,None,None
            if contains_illegal_indicators(json.dumps(img)+" "+(q or "")): return None,None,None,None
            extract_and_add_tags_from_meta(str(img.get("tags","")),GIF_TAGS,data)
            return gif_url,f"waifu_im_{q}",img,None
    except Exception as e:
...
async def fetch_from_waifu_it(session,positive):
    try:
        ...
        async with session.get(endpoint,headers=headers,timeout=REQUEST_TIMEOUT) as resp:
            ...
            if not gif_url: return None,None,None,None
            if filename_has_block_keyword(gif_url): return None,None,None,None
            if contains_illegal_indicators(json.dumps(payload)+" "+(q or "")): return None,None,None,None
            extract_and_add_tags_from_meta(json.dumps(payload),GIF_TAGS,data)
            return gif_url,f"waifu_it_{q}",payload,None
    except Exception as e:
...
async def fetch_from_nekos_best(session,positive):
    try:
        ...
        async with session.get(url,timeout=REQUEST_TIMEOUT) as resp:
            ...
            if not gif_url: return None,None,None,None
            if filename_has_block_keyword(gif_url): return None,None,None,None
            if contains_illegal_indicators(json.dumps(r)+" "+(q or "")): return None,None,None,None
            extract_and_add_tags_from_meta(json.dumps(r),GIF_TAGS,data)
            return gif_url,f"nekos_best_{q}",r,None
    except Exception as e:
...
async def fetch_from_nekos_life(session,positive):
    try:
        ...
        async with session.get(url,timeout=REQUEST_TIMEOUT) as resp:
            ...
            if contains_illegal_indicators(json.dumps(payload)+" "+(q or "")): return None,None,None,None
            extract_and_add_tags_from_meta(json.dumps(payload),GIF_TAGS,data)
            return gif_url,f"nekos_life_{q}",payload,None
    except Exception as e:
...
async def fetch_from_nekos_moe(session,positive):
    try:
        ...
        async with session.get(url,timeout=REQUEST_TIMEOUT) as resp:
            ...
            if not gif_url: return None,None,None,None
            if filename_has_block_keyword(gif_url): return None,None,None,None
            if contains_illegal_indicators(json.dumps(item)+" "+(q or "")): return None,None,None,None
            extract_and_add_tags_from_meta(json.dumps(item),GIF_TAGS,data)
            return gif_url,f"nekos_moe_{q}",item,None
    except Exception as e:
...
async def fetch_from_otakugifs(session,positive):
    try:
        ...
        async with session.get(url,timeout=REQUEST_TIMEOUT) as resp:
            ...
            if contains_illegal_indicators(json.dumps(payload)+" "+(q or "")): return None,None,None,None
            extract_and_add_tags_from_meta(json.dumps(payload),GIF_TAGS,data)
            return gif_url,f"otakugifs_{reaction}",payload,None
    except Exception as e:
...
async def fetch_from_animegirls_online(session,positive):
    try:
        ...
        async with session.get(url,timeout=REQUEST_TIMEOUT) as resp:
            ...
            if contains_illegal_indicators(json.dumps(payload)+" "+(q or "")): return None,None,None,None
            extract_and_add_tags_from_meta(json.dumps(payload),GIF_TAGS,data)
            return gif_url,f"animegirls_online_{q}",payload,None
    except Exception as e:
    ...
    try:
        ...
        async with session.get(url,params=params,timeout=REQUEST_TIMEOUT,auth=auth) as resp:
            ...
            for item in payload:
                ...
                return gif_url,f"danbooru_{q}",item,None
            return None,None,None,None
    except Exception as e:
...
PROVIDER_FETCHERS = {
    ...
    "otakugifs":fetch_from_otakugifs,"animegirls_online":fetch_from_animegirls_online,"danbooru":fetch_from_danbooru
}
...
async def attempt_get_media_bytes(session,gif_url):
    if not gif_url: return None,None,None,"no-url"
    if contains_illegal_indicators(gif_url): return None,None,None,"illegal-indicator-in-url"
    ...
    if status is None:
        ...
        if b: return b,ctype,gif_url,"downloaded-after-head-failed"
        return None,ctype,gif_url,"head-failed-get-failed"
    if status not in (200,301,302):
        b,ctype=await _download_bytes_with_limit(session,gif_url,size_limit=HEAD_SIZE_LIMIT)
        if b: return b,ctype,gif_url,f"get-after-head-{status}"
        return None,ctype,gif_url,f"head-{status}-get-failed"
    cl=headers.get("Content-Length") or headers.get("content-length")
    ctype=headers.get("Content-Type") or headers.get("content-type") or ""
    if cl:
        try:
            clv=int(cl)
            if clv>HEAD_SIZE_LIMIT: return None,ctype,gif_url,f"too-large-head-{clv}"
            b,ctype2=await _download_bytes_with_limit(session,gif_url,size_limit=HEAD_SIZE_LIMIT)
            if b: return b,ctype2 or ctype,gif_url,"downloaded-with-head-size"
            return None,ctype2 or ctype,gif_url,"head-said-small-but-get-failed"
        except Exception:
            b,ctype2=await _download_bytes_with_limit(session,gif_url,size_limit=HEAD_SIZE_LIMIT)
            if b: return b,ctype2 or ctype,gif_url,"downloaded-with-head-parse-except"
            return None,ctype2 or ctype,gif_url,"head-parse-get-failed"
    else:
        b,ctype2=await _download_bytes_with_limit(session,gif_url,size_limit=HEAD_SIZE_LIMIT)
        if b: return b,ctype2 or ctype,gif_url,"downloaded-unknown-size"
        return None,ctype2 or ctype,gif_url,"unknown-size-get-failed-or-too-large"
...
async def fetch_gif(user_id):
    ...
    if not providers:
        if DEBUG_FETCH: logger.debug("No providers"); return None,None,None,None
    async with aiohttp.ClientSession() as session:
        ...
        while attempt<FETCH_ATTEMPTS:
            ...
            pool=PROVIDER_TERMS.get(provider) or PROVIDER_TERMS.get(provider+"_safe")
            ...
            try:
                gif_url,name_hint,meta,_ = await fetcher(session,positive)
            except Exception as e:
                if DEBUG_FETCH: logger.debug(f"fetcher exception {provider}: {e}")
                continue
            if not gif_url:
            ...
            if gif_hash in sent:
                if DEBUG_FETCH: logger.debug("already sent hash; skipping"); continue
            b,ctype,url,reason = await attempt_get_media_bytes(session,gif_url)
        ...
        if DEBUG_FETCH: logger.debug("fetch_gif exhausted"); return None,None,None,None
...
def try_compress_bytes(b,ctype,max_size):
    ...
    try:
        ...
        if fmt.upper() in ("GIF","WEBP"):
            ...
            for i in range(10):
                out=io.BytesIO()
                new_size=(max(1,int(w*scale**i)),max(1,int(h*scale**i)))
                resized=[]
                for fr in frames:
                    resized.append(fr.resize(new_size,Image.LANCZOS))
                try:
            ...
            # static image: try decreasing quality / size
            w,h=img.size
            scale=0.9
            for i in range(10):
                out=io.BytesIO()
                ...
                if fmt.upper() in ("JPEG","JPG"):
                    img2.save(out,format="JPEG",quality=max(10,90-8*i),optimize=True)
                else:
            ...
            if gif_bytes:
                ...
                if compressed and len(compressed)<=max_upload:
                    try:
                        ...
                        logger.debug(f"attach after compress failed: {e}")
                        if text_channel:
                            if gif_url and gif_url not in (embed.description or ""):
                                embed.description=(embed.description or "")+f"\n\n[View media here]({gif_url})"
                            await text_channel.send(embed=embed)
                    ...
                    return
            # fallback: send embed with link only
            if gif_url and gif_url not in (embed.description or ""):
                embed.description=(embed.description or "")+f"\n\n[View media here]({gif_url})"
...
while len(JOIN_GREETINGS) < 100: JOIN_GREETINGS.append(random.choice(JOIN_GREETINGS))
...
while len(LEAVE_GREETINGS) < 100: LEAVE_GREETINGS.append(random.choice(LEAVE_GREETINGS))
    ...
    try:
        ...
        if not vc:
            try:
                ...
                if DEBUG_FETCH: logger.debug(f"failed to connect {e}")
        else:
...
async def on_ready():
    ...
    for p in PROVIDER_FETCHERS.keys():
        ...
        if p=="danbooru" and (not DANBOORU_API_KEY or not DANBOORU_USER): key_ok=False
...
async def on_voice_state_update(member,before,after):
    ...
    if before.channel and (before.channel.id in VC_IDS) and (after.channel!=before.channel):
        ...
        gif_bytes,gif_name,gif_url,ctype=await fetch_gif(member.id)
        await send_embed_with_media(text_channel,member,embed,gif_bytes,gif_name,gif_url,ctype)
        # bot stays 24/7 (do not disconnect)
