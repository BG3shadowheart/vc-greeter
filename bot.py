# spiciest-sfw-final.py
# Bot connects when user joins VC, disconnects when empty, and never re-sends the same GIF to the same user.
...
# optional for compression; if not installed, compression is skipped
try:
...
# Keep the exact VC IDs and text channel ID you specified
VC_IDS = [
    1353875050809524267,
...
logger = logging.getLogger("spiciest-sfw-final")
...
_seed_gif_tags = [
    ...
    "cosplay","maid","bikini","swimsuit","school_uniform","idol","thighs",
    "stockings","panties_peek","underboob_hint","fanservice","flirty","teasing",
    "dance","cheerleader","sundress","soft","petite","playful","romantic","beach"
]
...
EXCLUDE_TAGS = [
    ...
    "futa","futanari","shemale","dickgirl","femboy","trap","gay","yaoi","male","man","boy"
]
...
# Each returns (gif_url, name_hint, meta) or (None, None, None)
...
# Fetching media but DO NOT mark it as "sent" yet. The sent history must be updated only AFTER successful send.
...
# fetch_gif now avoids GIFs already sent to this user by filtering against sent_history,
# but does NOT mark as sent (the send marking happens after successful sending).
...
# --------- compression helper (optional if Pillow available) ----------
def try_compress_bytes(b, ctype, max_size):
    ...
    try:
        ...
        if fmt.upper() in ("GIF", "WEBP"):
            ...
            w, h = frames[0].size
            for scale_pct in [0.9, 0.8, 0.7, 0.6, 0.5]:
                out = io.BytesIO()
                new_size = (max(1, int(w * scale_pct)), max(1, int(h * scale_pct)))
...
# --------- embed & send; after sending successfully, record hash to prevent re-sending ---------
...
async def record_sent_for_user(member_id, gif_url):
    try:
        ...
        logger.debug(f"record_sent error: {e}")
...
async def send_embed_with_media(text_channel, member, embed, gif_bytes, gif_name, gif_url, ctype=None):
    ...
    try:
        if gif_bytes and len(gif_bytes) <= max_upload:
            ...
            # if bytes exist but too big, try compressing if PIL available
            if gif_bytes:
    ...
    # record sent if we had a gif_url and the send was successful (either attachment or link)
    if sent_success and gif_url:
        await record_sent_for_user(member.id, gif_url)

# --------- Greetings (unchanged; full 100 entries per list) ----------
JOIN_GREETINGS = [
...
# Connect when user joins; disconnect when monitored VC becomes empty (no non-bot members)
@bot.event
async def on_voice_state_update(member, before, after):
    ...
    if before.channel and (before.channel.id in VC_IDS) and (after.channel != before.channel):
        ...
        gif_bytes, gif_name, gif_url, ctype = await fetch_gif(member.id)
        await send_embed_with_media(text_channel, member, embed, gif_bytes, gif_name, gif_url, ctype)
        # check if bot is in that voice channel and if the channel is now empty of non-bot members; if so, disconnect
        try:
            vc = discord.utils.get(bot.voice_clients, guild=member.guild)
